...
from openai_server import (APIServer, anthropic_to_openai, anthropic_tools, APIError,
                           ...
                           render_chat_qwen38, render_chat_v4)
...
class FakeEngine:
    ...
    def generate(self, prompt, maximum, temperature, top_p, on_text, cache_slot=0,
                 ...
                 on_tool=None, presence_penalty=None):
...
class MessagesHTTPTest(unittest.TestCase):
    ...
    def test_qwen38_returns_anthropic_tool_use(self):
        self.engine.script = ("<tool_call>\n<function=get_weather>\n"
                              "<parameter=city>\nParis\n</parameter>\n"
                              "</function>\n</tool_call>",)
        body = self.base_body(tools=[{"name": "get_weather", "input_schema": {
            "type": "object", "properties": {"city": {"type": "string"}}}}])
        with patch("openai_server.ARCH", "qwen38"):
            with self.post(body) as response:
                payload = json.load(response)
        self.assertEqual(payload["stop_reason"], "tool_use")
        self.assertEqual(payload["content"], [{"type": "tool_use",
            "id": payload["content"][0]["id"], "name": "get_weather",
            "input": {"city": "Paris"}}])
