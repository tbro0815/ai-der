...
static void test_submit_and_controls(void)
{
    ...
    static const char reset[] = "RESET req-9 0\n";
    input = bytes_input(reset, sizeof(reset) - 1);
    assert(coli_serve_read_command(input, &profile, &command) == COLI_SERVE_READ_OK);
    assert(command.kind == COLI_SERVE_COMMAND_RESET);
    assert(strcmp(command.id, "req-9") == 0 && command.slot == 0);
    coli_serve_command_dispose(&command);
    fclose(input);
}
...
static void test_writer_golden_bytes(void)
{
    ...
    assert(coli_serve_write_reset(output, "req-9"));
    assert(coli_serve_write_accept(output, "req-7", 12));
    assert(coli_serve_write_prefill(output, "req-7", 12, 5, 0.25));
