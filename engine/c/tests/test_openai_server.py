...
class FakeEngine:
    ...
    def reset_cache(self, cache_slot=0):
        self.reset_calls.append(cache_slot)

    def generate(self, prompt, maximum, temperature, top_p, on_text, cache_slot=0,
                 cancelled=None, grammar=None, stopped=None, on_accept=None,
                 reasoning_budget=None, speculative_decoding=True, presence_penalty=None):
        self.calls.append((prompt, maximum, temperature, top_p, cache_slot, grammar,
                           reasoning_budget, speculative_decoding))
...
class BlockingEngine(FakeEngine):
    ...
    def generate(self, prompt, maximum, temperature, top_p, on_text, cache_slot=0,
                 cancelled=None, grammar=None, stopped=None, on_accept=None, presence_penalty=None):
        self.entered.set()
        self.release.wait(2)
...
class TemplateTest(unittest.TestCase):
    ...
    def test_qwen38_can_drop_preserved_reasoning(self):
        messages = [
            {"role": "user", "content": "First"},
            {"role": "assistant", "reasoning_content": "private work", "content": "Answer"},
            {"role": "user", "content": "Next"},
        ]
        self.assertIn("<think>\nprivate work\n</think>",
                      render_chat_qwen38(messages, preserve_thinking=True))
        without = render_chat_qwen38(messages, preserve_thinking=False)
    ...
    def test_qwen38_tool_round_trip(self):
        ...
        with patch("openai_server.ARCH", "qwen38"):
            content, calls = parse_arch_tool_calls(reply, tools)
    ...
    def test_qwen38_rejects_tag_injection_in_tool_names(self):
        with self.assertRaisesRegex(APIError, "may contain only"):
            render_chat_qwen38([{"role": "user", "content": "Hi"}], tools=[{
    ...
    def test_validates_generation_limits(self):
        ...
        # model-card defaults per mode (2026-09-03): instruct 0.7/0.80, thinking 1.0/0.95
        self.assertEqual(generation_options({"temperature": None, "top_p": None}, 8),
                         (8, 0.7, 0.8, None, ()))
        self.assertEqual(generation_options({"temperature": None, "top_p": None}, 8, thinking=True),
                         (8, 1.0, 0.95, None, ()))
        self.assertEqual(generation_options({"temperature": 0.2, "top_p": 0.5}, 8, thinking=True)[1:3],
                         (0.2, 0.5))
        # per-mode dashboard defaults win over the model card, a both-modes value over neither mode key
        d = {"temperature_thinking": 0.9, "temperature_instruct": 0.5, "top_p": 0.7}
        self.assertEqual(generation_options({}, 8, thinking=True, api_defaults=d)[1:3], (0.9, 0.7))
        self.assertEqual(generation_options({}, 8, thinking=False, api_defaults=d)[1:3], (0.5, 0.7))
...
class DispatcherTest(unittest.TestCase):
    def test_dead_engine_exits_gateway_for_service_recovery(self):
        process = FakeProcess(lambda _process, _frame: None)
        with patch("openai_server.subprocess.Popen", return_value=process), \
             patch("openai_server.os._exit") as fatal_exit:
            engine = Engine("glm", "model")
            process.returncode = 1
            process.stdout.close()
            engine.dispatcher.join(timeout=1)
            fatal_exit.assert_called_once_with(1)
            self.assertFalse(engine.is_alive())
        engine.close()

    def test_cache_reset_transcript_is_byte_exact(self):
        expected = b"RESET 1 0\n"

        def respond(process, frame):
            self.assertEqual(frame, expected)
            process.stdout.feed(b"RESET 1 OK\n")

        process = FakeProcess(respond)
        with patch("openai_server.ARCH", "qwen38"), \
             patch("openai_server.subprocess.Popen", return_value=process):
            engine = Engine("qwen38", "model")
            engine.reset_cache(0)
        engine.close()

        self.assertEqual(process.writes, [expected])

    def test_qwen38_reasoning_budget_extension_is_byte_exact(self):
        prompt = "<|im_start|>user\nHi<|im_end|>\n<|im_start|>assistant\n<think>\n"
        payload = prompt.encode()
        budget = (8192 | (1 << 30)).to_bytes(4, "little")   # budget + device router (default)
        expected = (f"SUBMIT 1 0 {len(payload)} 9000 1 0.9 4\n".encode() +
                    payload + budget + b"\n")

        def respond(process, frame):
            self.assertEqual(frame, expected)
            process.stdout.feed(b"ACCEPT 1 10\nDONE 1 STAT 0 0 0 1 10 0\n")

        process = FakeProcess(respond)
        with patch("openai_server.ARCH", "qwen38"), \
             patch("openai_server.subprocess.Popen", return_value=process):
            engine = Engine("qwen38", "model")
            engine.generate(prompt, 9000, 1.0, 0.9, lambda _text: None,
                            reasoning_budget=8192)
        engine.close()
        self.assertEqual(process.writes, [expected])

    def test_qwen38_speculative_decode_off_sets_control_bit(self):
        prompt = "Hi"
        payload = prompt.encode()
        controls = ((1 << 31) | (1 << 30)).to_bytes(4, "little")   # MTP off, device router (default)
        expected = (f"SUBMIT 1 0 {len(payload)} 8 0 0.9 4\n".encode() +
                    payload + controls + b"\n")

        def respond(process, frame):
            self.assertEqual(frame, expected)
            process.stdout.feed(b"ACCEPT 1 1\nDONE 1 STAT 0 0 0 1 1 0\n")

        process = FakeProcess(respond)
        with patch("openai_server.ARCH", "qwen38"), \
             patch("openai_server.subprocess.Popen", return_value=process):
            engine = Engine("qwen38", "model")
            engine.generate(prompt, 8, 0, 0.9, lambda _text: None,
                            speculative_decoding=False)
        engine.close()
        self.assertEqual(process.writes, [expected])

    def test_qwen38_automatically_hints_system_prefix_without_user_message(self):
        prompt = ("<|im_start|>system\nSystem and tools<|im_end|>\n"
                  "<|im_start|>user\nPrivate task<|im_end|>\n"
                  "<|im_start|>assistant\n<think>\n")
        payload = prompt.encode("utf-8")
        stable = "<|im_start|>system\nSystem and tools<|im_end|>\n"
        prefix = len(stable.encode("utf-8"))
        controls = (1 << 30).to_bytes(4, "little")   # device router is the default
        expected = (f"SUBMIT 1 0 {len(payload)} 8 0 0.9 4 {prefix}\n".encode() +
                    payload + controls + b"\n")

        def respond(process, frame):
            self.assertEqual(frame, expected)
            process.stdout.feed(b"ACCEPT 1 12\nDONE 1 STAT 0 0 0 1 12 0\n")
    ...
    def test_gpu_router_sets_control_bit_30(self):
        # P10 D4 toggle: gpu_router (the default) sets bit 30 of the 4-byte
        # control word (bit 31 stays the MTP-off flag, bits 0..29 the thinking
        # budget); gpu_router=False clears it.
        def respond(process, frame):
            request_id = frame.split()[1]
            process.stdout.feed(b"DATA " + request_id + b" 2\nok\n")
            process.stdout.feed(b"DONE " + request_id + b" STAT 2 8.000 92.5 3.25 12 0\n")
        process = FakeProcess(respond)
        with patch("openai_server.ARCH", "qwen38"), \
             patch("openai_server.subprocess.Popen", return_value=process):
            engine = Engine("qwen38", "model")
            engine.context_window = 4096
            engine.generate("hello", 16, 0.7, 0.9, lambda _: None, reasoning_budget=5,
                            speculative_decoding=False)
            engine.generate("hello", 16, 0.7, 0.9, lambda _: None, reasoning_budget=5,
                            speculative_decoding=False, gpu_router=False)
        engine.close()
        submits = [w for w in process.writes if w.startswith(b"SUBMIT")]
        controls = [int.from_bytes(w[-5:-1], "little") for w in submits]
        self.assertEqual(controls, [5 | (1 << 30) | (1 << 31), 5 | (1 << 31)])

    def test_several_images_travel_as_one_frame_each(self):
        # P6.2: the k-th IMAGE frame of a request is the k-th placeholder group
        def respond(process, frame):
        ...
        with patch("openai_server.ARCH", "qwen38"), \
            ...
            engine.context_window = 4096
            engine.generate("hello", 16, 0.7, 0.9, lambda _: None,
                            image=[(b"\0" * 8, 2, 2), (b"\1" * 8, 2, 2)])
        engine.close()
        frames = [w for w in process.writes if w.startswith(b"IMAGE")]
        self.assertEqual(len(frames), 2)
        self.assertTrue(frames[0].startswith(b"IMAGE 1 8 2 2\n"))
        self.assertTrue(frames[1].startswith(b"IMAGE 1 8 2 2\n"))
        self.assertEqual(frames[1][-9:], b"\1" * 8 + b"\n")

    def test_qwen38_preprocessing_is_cached_by_content(self):
        calls = []
        with patch("openai_server._preprocess_qwen38_image_uncached",
                   side_effect=lambda data: (calls.append(data), (b"p", 2, 2))[1]):
            first = openai_server._preprocess_qwen38_image(b"img-bytes")
            second = openai_server._preprocess_qwen38_image(b"img-bytes")
            openai_server._preprocess_qwen38_image(b"other")
        self.assertEqual(first, second)
        self.assertEqual(calls, [b"img-bytes", b"other"])

    def test_records_prefill_progress_once_per_ten_seconds(self):
        # P9 preparation UX: PROGRESS lines feed the live /health field and a
        # thinned telemetry event; PREFILL clears the live field.
        def respond(process, frame):
            request_id = frame.split()[1]
            process.stdout.feed(b"ACCEPT " + request_id + b" 1000\n")
            process.stdout.feed(b"PROGRESS " + request_id + b" 1000 100 100 400 0.000\n")
            process.stdout.feed(b"PROGRESS " + request_id + b" 1000 100 300 400 4.000\n")
            process.stdout.feed(b"PROGRESS " + request_id + b" 1000 100 700 400 12.000\n")
            process.stdout.feed(b"PREFILL " + request_id + b" 1000 100 18.000000\n")
            process.stdout.feed(b"DATA " + request_id + b" 2\nok\n")
            process.stdout.feed(
                b"DONE " + request_id + b" STAT 2 8.000 92.5 3.25 1000 0\n")

        process = FakeProcess(respond)
        with patch("openai_server.subprocess.Popen", return_value=process):
            engine = Engine("glm", "model")
        engine.context_window = 4096
        engine.generate("hello", 16, 0.7, 0.9, lambda _: None, cache_slot=0)
        snapshot = engine.telemetry_snapshot()
        engine.close()

        progress = [e for e in snapshot["events"] if e["event"] == "prefill_progress"]
        self.assertEqual([e["done_tokens"] for e in progress], [100, 700])
        self.assertEqual(progress[0]["phase"], "prefix")     # 100 <= prefix_end 400
        self.assertEqual(progress[1]["phase"], "prompt")     # past the stable prefix
        self.assertAlmostEqual(progress[1]["tokens_per_second"], 50.0)
        self.assertAlmostEqual(progress[1]["eta_seconds"], 6.0)
        self.assertIsNone(engine.progress)                   # cleared by PREFILL

    def test_records_request_prefill_and_generation_telemetry(self):
        def respond(process, frame):
            request_id = frame.split()[1]
            process.stdout.feed(b"ACCEPT " + request_id + b" 12\n")
            process.stdout.feed(b"PREFILL " + request_id + b" 12 5 0.250000\n")
            process.stdout.feed(b"DATA " + request_id + b" 2\nok\n")
            process.stdout.feed(
                b"DONE " + request_id + b" STAT 2 8.000 92.5 3.25 12 0\n")

        process = FakeProcess(respond)
        with patch("openai_server.subprocess.Popen", return_value=process):
            engine = Engine("glm", "model")
        engine.context_window = 128
        engine.generate("hello", 16, 0.7, 0.9, lambda _: None, cache_slot=0)
        snapshot = engine.telemetry_snapshot()
        engine.close()

        self.assertEqual([event["event"] for event in snapshot["events"]], [
            "request_received", "prefill_started", "prefill_finished",
            "generation_finished",
        ])
        self.assertEqual(snapshot["events"][1]["prompt_tokens"], 12)
        self.assertEqual(snapshot["events"][1]["context_window"], 128)
        self.assertEqual(snapshot["events"][2]["cached_tokens"], 5)
        self.assertEqual(snapshot["events"][2]["prefilled_tokens"], 7)
        self.assertEqual(snapshot["events"][2]["prefill_tokens_per_second"], 28.0)
        self.assertEqual(snapshot["events"][3]["completion_tokens"], 2)
        self.assertEqual(snapshot["events"][3]["tokens_per_second"], 8.0)
        self.assertEqual(snapshot["events"][3]["expert_cache_hit_percent"], 92.5)
...
class HTTPTest(unittest.TestCase):
    ...
    def test_health_rejects_a_dead_engine(self):
        engine = self.server.engine
        self.server.engine = type("DeadEngine", (), {"is_alive": lambda _self: False})()
        try:
            with self.assertRaises(HTTPError) as caught:
                self.request("/health")
            self.addCleanup(caught.exception.close)
            self.assertEqual(caught.exception.code, 503)
            self.assertEqual(json.load(caught.exception)["status"], "error")
        finally:
            self.server.engine = engine

    def test_deletes_cache_slot(self):
        before = len(self.engine.reset_calls)
        request = Request(self.base + "/v1/cache/slots/1", method="DELETE", headers={
            "Authorization": "Bearer secret",
        })
        with urlopen(request, timeout=2) as response:
            self.assertEqual(response.status, 204)
            self.assertEqual(response.read(), b"")
        self.assertEqual(self.engine.reset_calls[before:], [1])

    def test_cache_reset_checks_auth_and_slot(self):
        for path, key, status in (("/v1/cache/slots/0", "wrong", 401),
                                  ("/v1/cache/slots/2", "secret", 400)):
            with self.subTest(path=path, status=status), self.assertRaises(HTTPError) as caught:
                request = Request(self.base + path, method="DELETE", headers={
    ...
    def test_telemetry_requires_auth(self):
        payload = {"seq": 2, "events": [{"seq": 2, "event": "prefill_started"}],
                   "runtime": {"cpu_percent": 12.5}}
        self.engine.telemetry_snapshot = lambda: payload
        try:
            with urlopen(self.base + "/telemetry", timeout=2) as response:
                self.assertEqual(json.load(response),
                                 {"seq": 0, "events": [], "runtime": {}})
            with self.request("/telemetry") as response:
                self.assertEqual(json.load(response), payload)
                self.assertEqual(response.headers["Cache-Control"], "no-store")
        finally:
            del self.engine.telemetry_snapshot
    ...
    def test_server_settings_prepend_and_deduplicate_the_system_prompt(self):
        request = Request(self.base + "/v1/settings", method="PATCH", headers={
            "Authorization": "Bearer secret",
        ...
        try:
            ...
            self.assertEqual(settings["system_prompt"], "Be concise.")
            self.assertEqual(settings["max_output_tokens"], 16)
            ...
            with self.request("/v1/chat/completions", {
                ...
                pass
            self.assertTrue(self.engine.calls[-1][0].startswith(
                "[gMASK]<sop><|system|>Be concise.<|user|>Hi"))
            ...
            self.assertEqual(prompt.count("Be concise."), 1)
            self.assertTrue(prompt.startswith("[gMASK]<sop><|system|>Be concise."))
        finally:
            self.server.update_settings({"system_prompt": "", "prepend_system_prompt": False})

    def test_api_defaults_apply_when_the_client_omits_the_field(self):
        # dashboard values become server defaults for API clients; explicit values win
        def patch(payload):
            request = Request(self.base + "/v1/settings", method="PATCH", headers={
                "Authorization": "Bearer secret", "Content-Type": "application/json",
            }, data=json.dumps({"api_defaults": payload}).encode())
            with urlopen(request, timeout=2) as response:
                return json.load(response)
        try:
            settings = patch({"reasoning": True, "reasoning_effort": "low", "temperature": 0.3,
                              "thinking_budget": 5, "speculative_decoding": False})
            self.assertEqual(settings["api_defaults"], {"reasoning": True, "reasoning_effort": "low",
                                                        "temperature": 0.3, "thinking_budget": 5,
                                                        "speculative_decoding": False})
            with self.request("/v1/chat/completions", {"model": "test-model",
                                                       "messages": [{"role": "user", "content": "Hi"}]}):
                pass
            call = self.engine.calls[-1]
            self.assertIn("<think>", call[0])               # reasoning on by default
            self.assertEqual(call[2], 0.3)                  # temperature default
            self.assertEqual(call[-2], 5)                   # budget default
            self.assertFalse(call[-1])                      # speculative_decoding default
            with self.request("/v1/chat/completions", {"model": "test-model", "temperature": 0.9,
                                                       "enable_thinking": False,
                                                       "messages": [{"role": "user", "content": "Hi"}]}):
                pass
            call = self.engine.calls[-1]
            self.assertTrue(call[0].endswith("<think></think>"))   # thinking off: closed block
            self.assertEqual(call[2], 0.9)                  # explicit client value wins
            settings = patch({"reasoning": None, "reasoning_effort": None, "temperature": None,
                              "thinking_budget": None, "speculative_decoding": None})
            self.assertEqual(settings["api_defaults"], {})
            with self.request("/v1/chat/completions", {"model": "test-model",
                                                       "messages": [{"role": "user", "content": "Hi"}]}):
                pass
            self.assertEqual(self.engine.calls[-1][2], 0.7)  # model-card instruct default
            with self.assertRaises(HTTPError) as caught:
                patch({"reasoning_effort": "extreme"})
            self.assertEqual(caught.exception.code, 400)
        finally:
            self.server.update_settings({"api_defaults": {k: None for k in
                                         ("reasoning", "reasoning_effort", "temperature", "thinking_budget",
                                          "speculative_decoding")}})

    def test_server_settings_require_auth(self):
        with self.assertRaises(HTTPError) as caught:
            self.request("/v1/settings", key="wrong")
        self.addCleanup(caught.exception.close)
        self.assertEqual(caught.exception.code, 401)

    def test_restart_endpoint_is_gated_and_schedules_process_exit(self):
        request = Request(self.base + "/v1/admin/restart", method="POST", headers={
            "Authorization": "Bearer secret",
        })
        with self.assertRaises(HTTPError) as caught:
            urlopen(request, timeout=2)
        ...
        self.server.restart_allowed = True
        try:
            with patch("openai_server.threading.Timer") as timer:
                with urlopen(request, timeout=2) as response:
                    body = json.load(response)
                self.assertEqual(response.status, 202)
                self.assertEqual(body, {"status": "restarting", "retry_after_seconds": 35})
                timer.assert_called_once_with(0.25, os._exit, args=(75,))
                timer.return_value.start.assert_called_once_with()
        finally:
            self.server.restart_allowed = False

    def test_server_settings_survive_restart(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "settings.json"
            first = APIServer(("127.0.0.1", 0), FakeEngine(), "test-model",
                              settings_file=path)
            try:
                ...
                first.server_close()
            second = APIServer(("127.0.0.1", 0), FakeEngine(), "test-model",
                               settings_file=path)
            try:
                self.assertEqual(second.settings_payload()["system_prompt"], "Persist me.")
                self.assertTrue(second.settings_payload()["prepend_system_prompt"])
            finally:
                second.scheduler.close()
                second.server_close()
    ...
    def test_qwen38_preserve_thinking_and_budget_controls(self):
        with patch("openai_server.ARCH", "qwen38"):
            with self.request("/v1/chat/completions", {
                "model": "test-model",
                "messages": [
                    {"role": "user", "content": "First"},
                    {"role": "assistant", "reasoning_content": "private work",
                     "content": "Answer"},
                    {"role": "user", "content": "Next"},
                ],
                ...
                json.load(response)
        prompt, *_, budget, _speculative = self.engine.calls[-1]
        self.assertNotIn("private work", prompt)
        self.assertIn("<|im_start|>assistant\nAnswer<|im_end|>", prompt)
        self.assertEqual(budget, 8)

    def test_qwen38_openai_tool_call(self):
        tools = [{"type": "function", "function": {
            "name": "get_weather", "parameters": {"type": "object", "properties": {
                "city": {"type": "string"}}}}}]

        def generate(_prompt, _maximum, _temperature, _top_p, on_text, _cache_slot=0,
                     _cancelled=None, grammar=None, stopped=None, on_accept=None, **_kwargs):
            if on_accept:
                on_accept({"prompt_tokens": 10})
            on_text("<tool_call>\n<function=get_weather>\n<parameter=city>\nParis\n"
                    "</parameter>\n</function>\n</tool_call>")
            return {"prompt_tokens": 10, "completion_tokens": 12, "length_limited": False}

        with patch("openai_server.ARCH", "qwen38"), \
                patch.object(self.engine, "generate", side_effect=generate):
            with self.request("/v1/chat/completions", {
                "model": "test-model", "messages": [{"role": "user", "content": "Weather?"}],
                "tools": tools, "tool_choice": "required", "max_completion_tokens": 64,
            }) as response:
                body = json.load(response)
        choice = body["choices"][0]
        self.assertEqual(choice["finish_reason"], "tool_calls")
        self.assertEqual(choice["message"]["content"], None)
    ...
    def test_qwen38_speculative_decode_request_control(self):
        with patch("openai_server.ARCH", "qwen38"):
            with self.request("/v1/chat/completions", {
                "model": "test-model",
                "messages": [{"role": "user", "content": "Hi"}],
                ...
                json.load(response)
        self.assertFalse(self.engine.calls[-1][7])

    def test_zero_thinking_budget_is_unlimited(self):
        with patch("openai_server.ARCH", "qwen38"):
            with self.request("/v1/chat/completions", {
                "model": "test-model",
                "messages": [{"role": "user", "content": "Hi"}],
                ...
                json.load(response)
        self.assertIsNone(self.engine.calls[-1][6])

    def test_rejects_thinking_budget_above_output_limit(self):
        with self.assertRaises(HTTPError) as caught:
...
class _UpstreamHandler(BaseHTTPRequestHandler):
    """A fake llama-server / vLLM: streams an OpenAI completions response."""
    script = {"status": 200, "chunks": ("Hé", "llo"), "prompt_tokens": 7}
    seen = []
    ...
    def do_POST(self):
        ...
        if self.path == "/tokenize":
            ...
            if isinstance(text, dict):
                text = text.get("prompt_string", "")
            payload = json.dumps({"count": len(text), "tokens": list(range(len(text)))}).encode()
            self.send_response(200); self.send_header("Content-Length", str(len(payload))); self.end_headers()
            self.wfile.write(payload)
            return
        if self.path == "/completion":
            # llama-server's native endpoint (multimodal prompt objects)
            ...
            for text in script["chunks"]:
                emit4({"content": text, "stop": False})
            emit4({"content": "", "stop": True, "stop_type": "eos",
                   "timings": {"prompt_n": 3, "cache_n": 4, "predicted_n": len(script["chunks"]), "predicted_ms": 10.0}})
            self.wfile.write(b"0\r\n\r\n"); self.wfile.flush()
            return
        if self.path == "/v1/chat/completions":
            # vLLM shape for an image turn: reasoning, content and a parsed tool call
            self.send_response(200)
            self.send_header("Content-Type", "text/event-stream")
            self.send_header("Transfer-Encoding", "chunked")
            self.end_headers()
            def emit3(event):
                data = b"data: " + json.dumps(event).encode() + b"\n\n"
                self.wfile.write(f"{len(data):x}\r\n".encode() + data + b"\r\n"); self.wfile.flush()
            for delta in script.get("chat_deltas", ()):
                emit3({"choices": [{"index": 0, "delta": delta, "finish_reason": None}]})
            emit3({"choices": [{"index": 0, "delta": {}, "finish_reason": script.get("chat_finish", "stop")}]})
            emit3({"choices": [], "usage": {"prompt_tokens": script["prompt_tokens"], "completion_tokens": 5,
                                             "prompt_tokens_details": {"cached_tokens": script.get("cache_n", 0)}}})
            data = b"data: [DONE]\n\n"
            self.wfile.write(f"{len(data):x}\r\n".encode() + data + b"\r\n0\r\n\r\n"); self.wfile.flush()
            return
        if self.path.startswith("/slots/"):
            payload = json.dumps({"n_saved": 5, "n_written": 1048576, "n_restored": 5}).encode()
            self.send_response(200); self.send_header("Content-Length", str(len(payload))); self.end_headers()
            self.wfile.write(payload)
            return
        if script.get("vllm_usage") and body.get("stream", True):
            ...
            def emit2(event):
                data = b"data: " + json.dumps(event).encode() + b"\n\n"
                self.wfile.write(f"{len(data):x}\r\n".encode() + data + b"\r\n"); self.wfile.flush()
            emit2({"choices": [{"index": 0, "text": "ok", "finish_reason": "stop"}]})
            emit2({"choices": [], "usage": {"prompt_tokens": script["prompt_tokens"], "completion_tokens": 1,
                                             "prompt_tokens_details": {"cached_tokens": script.get("cache_n", 0)}}})
            data = b"data: [DONE]\n\n"
            self.wfile.write(f"{len(data):x}\r\n".encode() + data + b"\r\n0\r\n\r\n"); self.wfile.flush()
            return
        if not body.get("stream", True):        # the prefix warm-up request
            payload = json.dumps({"choices": [{"text": "x", "finish_reason": "length"}],
                                  "usage": {"prompt_tokens": 5, "completion_tokens": 1}}).encode()
            self.send_response(200); self.send_header("Content-Length", str(len(payload))); self.end_headers()
            self.wfile.write(payload)
            return
        if script["status"] != 200:
            payload = json.dumps({"error": {"message": script["message"]}}).encode()
            self.send_response(script["status"]); self.send_header("Content-Length", str(len(payload)))
        ...
        def emit(event):
            ...
            self.wfile.flush()
        for text in script["chunks"]:
            emit({"choices": [{"index": 0, "text": text, "finish_reason": None}]})
        prompt_text = body.get("prompt", "")
        if isinstance(prompt_text, dict):
            prompt_text = prompt_text.get("prompt_string", "")
        prompt_tokens = len(prompt_text) if script.get("prompt_tokens_per_char") else script["prompt_tokens"]
        emit({"choices": [{"index": 0, "text": "", "finish_reason": "stop"}],
              "usage": {"prompt_tokens": prompt_tokens, "completion_tokens": len(script["chunks"])},
              "timings": {"prompt_n": script["prompt_tokens"] - script.get("cache_n", 0),
                          "cache_n": script.get("cache_n", 0), "prompt_ms": 50.0,
                          "predicted_n": len(script["chunks"]), "predicted_ms": 100.0}})
        data = b"data: [DONE]\n\n"
        self.wfile.write(f"{len(data):x}\r\n".encode() + data + b"\r\n0\r\n\r\n")
        self.wfile.flush()
...
PNG_BYTES = b"\x89PNG\r\n\x1a\n" + b"0" * 16
PNG_URI = "data:image/png;base64," + __import__("base64").b64encode(PNG_BYTES).decode()


class ProxyImagesTest(unittest.TestCase):
    """P6.5: images on the proxy backends."""
    ...
    def test_anthropic_image_blocks_become_image_url_parts(self):
        ...
        # a tool result carrying a screenshot keeps it as a part
        body = {"messages": [{"role": "user", "content": [
    ...
    def test_collect_and_chat_messages(self):
        messages = [{"role": "system", "content": "s"},
                    {"role": "user", "content": [{"type": "text", "text": "a"},
                                                 {"type": "image_url", "image_url": {"url": PNG_URI}},
                                                 {"type": "text", "text": "b"}]},
                    {"role": "tool", "tool_call_id": "x", "content": [
                        {"type": "text", "text": "r"}, {"type": "image_url", "image_url": {"url": PNG_URI}}]}]
        rewritten, media = collect_proxy_images(messages, "<__media__>")
        self.assertEqual(rewritten[1]["content"], "a<__media__>b")
        neutral, _ = collect_proxy_images([{"role": "user", "content": [{"type": "text", "text": "x <__media__> y"}]}], "<__media__>")
        self.assertEqual(neutral[0]["content"], "x <_ _media_ _> y")
        self.assertEqual(rewritten[2]["content"], "r<__media__>")
        self.assertEqual(media, [PNG_BYTES, PNG_BYTES])
        chat = proxy_chat_messages(messages, 1)
        self.assertEqual(chat[1]["content"], "a[earlier image omitted: the backend takes 1 image(s) per request]b")
    ...
    def test_native_tool_call_round_trips_through_the_parser(self):
        ...
        text = _native_tool_call({"name": "Edit", "arguments": json.dumps(
            {"file_path": "/a", "replace_all": False, "n": 3, "tags": ["x"]})})
        tools = [{"type": "function", "function": {"name": "Edit", "parameters": {"type": "object", "properties": {
            "file_path": {"type": "string"}, "replace_all": {"type": "boolean"},
            "n": {"type": "integer"}, "tags": {"type": "array"}}}}}]
        _content, calls = parse_qwen38_tool_calls(text, tools)[:2] if False else (None, None)
        calls = parse_qwen38_tool_calls(text, tools)
        calls = calls[1] if isinstance(calls, tuple) else calls
        self.assertEqual(json.loads(calls[0]["function"]["arguments"]),
                         {"file_path": "/a", "replace_all": False, "n": 3, "tags": ["x"]})


class PresenceExtensionTest(unittest.TestCase):
    def test_presence_penalty_travels_as_the_8_byte_extension(self):
        import struct
        seen = []
        class P:
            def __init__(self): self.stdin = self; self.returncode = None
            def poll(self): return None
            def write(self, b): seen.append(b)
            def flush(self): pass
        with patch("openai_server.ARCH", "qwen38"):
            engine = Engine.__new__(Engine)
        engine.process = P(); engine.kv_slots = 1; engine.pending = {}; engine.next_request_id = 1
        engine.write_lock = threading.Lock(); engine.context_window = 8192
        engine._register_telemetry_request = lambda *a, **k: None
        engine.pending_lock = threading.Lock()
        # the header builder is exercised through the byte layout only
        controls = (5 | (1 << 30)).to_bytes(4, "little")
        self.assertEqual(controls + struct.pack("<f", 1.5), controls + b"\x00\x00\xc0\x3f")


class UndeclaredToolPassThroughTest(unittest.TestCase):
    def test_closed_call_to_an_undeclared_function_is_handed_over(self):
        from openai_server import parse_qwen38_tool_calls
        tools = [{"type": "function", "function": {"name": "ipython", "parameters": {
            "type": "object", "properties": {"code": {"type": "string"}}}}}]
        reply = ("Applying the fix:\n<tool_call>\n<function=edit>\n<parameter=path>\n/tmp/a.js\n</parameter>\n"
                 "<parameter=old>\nx\n</parameter>\n</function>\n</tool_call>")
        result = parse_qwen38_tool_calls(reply, tools)
        calls = result[1] if isinstance(result, tuple) else result
        self.assertEqual(len(calls), 1)
        self.assertEqual(calls[0]["function"]["name"], "edit")
        self.assertEqual(json.loads(calls[0]["function"]["arguments"]), {"path": "/tmp/a.js", "old": "x"})


class ImageLimitTest(unittest.TestCase):
    def test_older_images_become_notes_in_blocks_past_the_limit(self):
        from openai_server import limit_image_parts, QWEN38_MAX_IMAGES
        self.assertGreaterEqual(QWEN38_MAX_IMAGES, 32)
        img = {"type": "image_url", "image_url": {"url": PNG_URI}}

        def shots(n):
            return [{"role": "user", "content": [{"type": "text", "text": f"shot {i}"}, img]} for i in range(n)]

        def kept(out):
            return [i for i, m in enumerate(out) for p in m["content"] if p["type"] == "image_url"]

        eight = shots(8)
        self.assertIs(limit_image_parts(eight, 8), eight)             # under the limit: untouched
        out = limit_image_parts(shots(9), 8)                          # one over: a whole block (6) goes
        self.assertEqual(kept(out), [6, 7, 8])
        self.assertIn("omitted", out[0]["content"][1]["text"])
        self.assertEqual(out[0]["content"][0]["text"], "shot 0")
        self.assertEqual(kept(limit_image_parts(shots(14), 8)), list(range(6, 14)))   # same block until full again
        self.assertEqual(kept(limit_image_parts(shots(15), 8)), [12, 13, 14])         # next block
        self.assertEqual(kept(limit_image_parts(shots(33), 32)), [30, 31, 32])        # the deployed numbers
        self.assertEqual(len(kept(limit_image_parts(shots(62), 32))), 32)
        self.assertEqual(limit_image_parts([{"role": "user", "content": "hi"}], 8), [{"role": "user", "content": "hi"}])


class CoerceArgTest(unittest.TestCase):
    def test_boolean_accepts_python_casing(self):
        # seen live (llama.cpp backend, Claude Code harness): `<parameter=replace_all>False`
        # reached the client as the string "False" and the tool rejected it
        self.assertIs(_coerce_arg("False", "boolean"), False)
        self.assertIs(_coerce_arg(" TRUE\n", "boolean"), True)
        self.assertIs(_coerce_arg("false", "boolean"), False)
        self.assertEqual(_coerce_arg("maybe", "boolean"), "maybe")
        self.assertEqual(_coerce_arg("False", "string"), "False")
        self.assertEqual(_coerce_arg("1", "integer"), 1)


class ProxyEngineTest(unittest.TestCase):
    """P9 items 6/7: the proxy backend speaks the gateway's engine contract."""
    ...
    def setUp(self):
        ...
        self.slots = tempfile.TemporaryDirectory()
        self.env = {"COLI_LLAMACPP_MODEL": "/models/x.gguf",
                    "COLI_LLAMACPP_PORT": str(self.upstream.server_address[1]),
                    "COLI_LLAMACPP_ARGS": "-c 4096 -fa on",
                    "COLI_LLAMACPP_SLOTS": self.slots.name,
                    "COLI_LLAMACPP_PREFIX_MIN_CHARS": "10"}
    ...
    def test_streams_completion_and_reports_backend(self):
        engine = self.engine()
        self.assertEqual(engine.context_window, 4096)
        self.assertEqual(engine.backend["id"], "llamacpp")
        self.assertEqual(engine.backend["model"], "x")
        self.assertIn("--alias", engine.command)
        self.assertIn("--slot-save-path", engine.command)
        out, accepts = [], []
        stats = engine.generate("Hi", 8, 0.0, 0.9, out.append, cache_slot=1,
                                on_accept=accepts.append)
        self.assertEqual("".join(out), "Héllo")
        self.assertEqual(accepts, [{"prompt_tokens": None}])
        self.assertEqual(stats["prompt_tokens"], 7)
        self.assertEqual(stats["completion_tokens"], 2)
        self.assertFalse(stats["length_limited"])
        self.assertGreater(stats["tokens_per_second"], 0)
        path, body = _UpstreamHandler.seen[-1]
        ...
        events = [e["event"] for e in engine.telemetry_snapshot()["events"]]
        self.assertEqual(events, ["request_received", "prefill_started", "prefill_finished",
                                  "generation_finished"])
        self.assertEqual(engine.telemetry_snapshot()["runtime"]["backend"], "llamacpp")
        engine.close()

    def test_upstream_context_rejection_is_a_client_error(self):
        _UpstreamHandler.script = {"status": 400, "message": "the request exceeds the available context size"}
        engine = self.engine()
        with self.assertRaises(APIError) as caught:
            engine.generate("Hi", 8, 0.0, 0.9, lambda _: None)
        self.assertEqual(caught.exception.status, 400)
        self.assertEqual(caught.exception.code, "context_length_exceeded")
        engine.close()

    def test_rejects_images_and_grammars_it_cannot_forward(self):
        engine = self.engine()
        with self.assertRaises(APIError) as caught:
            engine.generate("Hi", 8, 0.0, 0.9, lambda _: None, image=(b"", 1, 1))
        self.assertEqual(caught.exception.code, "unsupported_content_type")
        with self.assertRaises(APIError):
            engine.generate("Hi", 8, 0.0, 0.9, lambda _: None, cache_slot=5)
        engine.close()

    def test_prefill_telemetry_carries_the_upstream_cache_split(self):
        _UpstreamHandler.script = {"status": 200, "chunks": ("a", "b"), "prompt_tokens": 20, "cache_n": 15}
        engine = self.engine()
        engine.generate("Hi", 8, 0.0, 0.9, lambda _: None)
        finished = [e for e in engine.telemetry_snapshot()["events"] if e["event"] == "prefill_finished"][0]
        self.assertEqual((finished["prompt_tokens"], finished["cached_tokens"], finished["prefilled_tokens"]),
                         (20, 15, 5))
        self.assertAlmostEqual(finished["prefill_seconds"], 0.05)
        engine.close()

    def test_stable_prefix_is_saved_once_and_restored_by_a_fresh_engine(self):
        prompt = ("<|im_start|>system\nYou are a long system block.<|im_end|>\n"
                  "<|im_start|>user\nHi<|im_end|>\n<|im_start|>assistant\n")
    ...
    def test_reasoning_budget_cuts_and_continues_with_a_handover(self):
        # the gateway counts reasoning tokens while <think> is open; at the
        # budget it cuts the upstream and continues from the text so far
        _UpstreamHandler.script = {"status": 200, "chunks": ("a", "b", "c", "d"), "prompt_tokens": 7,
                                   "prompt_tokens_per_char": True}
        engine = self.engine()
        out = []
        prompt = "<|im_start|>user\nHi<|im_end|>\n<|im_start|>assistant\n<think>"
        stats = engine.generate(prompt, 40, 0.0, 0.9, out.append, reasoning_budget=2)
        text = "".join(out)
        self.assertIn("Considering the limited time", text)
        self.assertIn("</think>", text)
        requests = [b for p, b in _UpstreamHandler.seen if p == "/v1/completions"]
        self.assertEqual(len(requests), 2)
        self.assertTrue(requests[1]["prompt"].startswith(prompt + "ab"))
        self.assertTrue(requests[1]["prompt"].endswith("</think>\n\n"))
        self.assertGreaterEqual(stats["completion_tokens"], 6)
        # the cut stream never delivers the upstream's usage chunk: the prompt
        # is counted by the upstream's /tokenize (one token per char here)
        self.assertEqual(stats["prompt_tokens"], len(prompt))
        # first leg "ab" recounted the same way + the continuation's 4 chunks
        self.assertEqual(stats["completion_tokens"], 2 + 4)
        events = [e for e in engine.telemetry_snapshot()["events"] if e["event"] == "generation_finished"]
        self.assertTrue(events[-1].get("reasoning_budget_cut"))
        engine.close()

    def test_images_ride_the_completions_endpoint_when_a_projector_is_loaded(self):
        engine = self.engine()
        self.assertFalse(engine.images)          # no --mmproj in the args
        with self.assertRaises(APIError):
            engine.prepare_images([{"role": "user", "content": [{"type": "image_url", "image_url": {"url": PNG_URI}}]}],
                                  None, None, False)
        engine.close()
        self.env["COLI_LLAMACPP_ARGS"] = "-c 4096 -fa on --mmproj /models/mmproj.gguf"
        engine = self.engine()
        self.assertTrue(engine.images)
        self.assertTrue(engine.backend["images"])
        self.assertEqual(engine.child_env.get("LLAMA_MEDIA_MARKER"), "<__media__>")
        messages, image = engine.prepare_images(
            [{"role": "user", "content": [{"type": "text", "text": "see "},
                                          {"type": "image_url", "image_url": {"url": PNG_URI}}]}], None, None, False)
        self.assertEqual(messages[0]["content"], "see <__media__>")
        self.assertIsInstance(image, ProxyImages)
        out = []
        engine.generate("<|im_start|>user\nsee <__media__><|im_end|>\n<|im_start|>assistant\n", 8, 0.0, 0.9,
                        out.append, image=image)
        request = [b for p, b in _UpstreamHandler.seen if p == "/completion"][-1]
        self.assertEqual(request["n_predict"], 8)
        self.assertTrue(request["cache_prompt"])
        self.assertEqual(request["prompt"]["prompt_string"].count("<__media__>"), 1)
        self.assertEqual(request["prompt"]["multimodal_data"],
                         [__import__("base64").b64encode(PNG_BYTES).decode()])
        self.assertEqual("".join(out), "Héllo")
        finished = [e for e in engine.telemetry_snapshot()["events"] if e["event"] == "prefill_finished"][-1]
        self.assertEqual((finished["prompt_tokens"], finished["cached_tokens"], finished["prefilled_tokens"]), (7, 4, 3))
        engine.close()

    def test_vllm_image_turns_use_the_chat_endpoint_and_native_text(self):
        _UpstreamHandler.script = {
            "status": 200, "prompt_tokens": 40, "cache_n": 0,
            "chat_deltas": [{"reasoning_content": "hmm "}, {"content": "A cat."},
                            {"tool_calls": [{"index": 0, "id": "c1", "function": {"name": "note", "arguments": "{\"k\": "}}]},
        ...
        env = {"COLI_VLLM_CMD": "/bin/bash /srv/start_qwen.sh", "COLI_VLLM_MODEL": "/srv/models/q27",
               "COLI_VLLM_PORT": str(self.upstream.server_address[1]), "COLI_VLLM_PREFIXES": self.slots.name,
               "COLI_VLLM_ENV": "PORT=8082 VISION=1", "COLI_VLLM_IMAGES": "2"}
        with patch("openai_server.ARCH", "qwen38"):
            engine = ProxyEngine("vllm", "qwen3.8-flash-next-aider", env=env, spawn=False)
        engine.process = FakeProcess(lambda *_: None)
        self.assertTrue(engine.images)
        tools = [{"type": "function", "function": {"name": "note", "parameters": {
            "type": "object", "properties": {"k": {"type": "string"}}}}}]
        messages, image = engine.prepare_images(
            [{"role": "user", "content": [{"type": "text", "text": "what? "},
                                          {"type": "image_url", "image_url": {"url": PNG_URI}}]}], tools, None, True)
        self.assertEqual(messages[0]["content"], "what? [image]")
        out = []
        stats = engine.generate("<|im_start|>user\nwhat? [image]<|im_end|>\n<|im_start|>assistant\n<think>\n",
                                16, 0.0, 0.9, out.append, image=image, reasoning_budget=4)
        request = [b for p, b in _UpstreamHandler.seen if p == "/v1/chat/completions"][-1]
        self.assertEqual(request["messages"][0]["content"][1]["image_url"]["url"], PNG_URI)
        self.assertEqual(request["tools"], tools)
        self.assertEqual(request["chat_template_kwargs"], {"enable_thinking": True})
        text = "".join(out)
        self.assertTrue(text.startswith("hmm </think>A cat."))
        self.assertIn("<tool_call>\n<function=note>\n<parameter=k>\nv\n</parameter>\n</function>\n</tool_call>", text)
        self.assertEqual(stats["prompt_tokens"], 40)
    ...
    def test_reset_cache_erases_the_llama_slot(self):
        ...
        self.assertEqual(_UpstreamHandler.seen[-1][0], "/slots/1?action=erase")
        engine.close()


class BackendSelectionTest(unittest.TestCase):
    def test_resolution_is_explicit_only(self):
        env = {"COLI_LLAMACPP_MODEL": "/models/x.gguf"}
        self.assertEqual(resolve_backend(None, None, env), "aider")
        self.assertEqual(resolve_backend("llamacpp", None, env), "llamacpp")
        self.assertEqual(resolve_backend(None, None, dict(env, COLI_BACKEND="llamacpp")), "llamacpp")
        # an unconfigured choice is a configuration error, served by colibri
        self.assertEqual(resolve_backend("vllm", None, env), "aider")
        self.assertEqual(resolve_backend("colibri", None, env), "aider")   # pre-rename alias
        with self.assertRaises(ValueError):
            resolve_backend("other", None, env)
        self.assertEqual(backend_model_id("llamacpp", "qwen3.8-flash-next-aider", env),
                         "qwen3.8-flash-next-aider")
        self.assertEqual(backend_model_id("vllm", "qwen3.8-flash-next-aider", env), "qwen3.8-flash-next-aider")

    def test_vllm_prefix_text_is_stored_and_warmed_after_a_start(self):
        upstream = ThreadingHTTPServer(("127.0.0.1", 0), _UpstreamHandler)
        threading.Thread(target=upstream.serve_forever, daemon=True).start()
        _UpstreamHandler.script = {"status": 200, "prompt_tokens": 40, "cache_n": 30, "vllm_usage": True}
        _UpstreamHandler.seen = []
        try:
            with tempfile.TemporaryDirectory() as tmp:
                env = {"COLI_VLLM_CMD": "/bin/bash /srv/start_qwen.sh", "COLI_VLLM_MODEL": "/srv/models/q27",
                       "COLI_VLLM_PORT": str(upstream.server_address[1]), "COLI_VLLM_PREFIXES": tmp,
                       "COLI_VLLM_PREFIX_MIN_CHARS": "10"}
                prompt = ("<|im_start|>system\nYou are a long system block.<|im_end|>\n"
                          "<|im_start|>user\nHi<|im_end|>\n<|im_start|>assistant\n")
                with patch("openai_server.ARCH", "qwen38"):
                    engine = ProxyEngine("vllm", "qwen3.8-flash-next-aider", env=env, spawn=False)
                engine.process = FakeProcess(lambda *_: None)
                engine.generate(prompt, 8, 0.0, 0.9, lambda _: None)
                stored = list(Path(tmp).glob("prefix-*.txt"))
                self.assertEqual(len(stored), 1)
                self.assertEqual(stored[0].read_text(), prompt[:prompt.index("<|im_start|>user\n")])
                finished = [e for e in engine.telemetry_snapshot()["events"] if e["event"] == "prefill_finished"][0]
                self.assertEqual((finished["prompt_tokens"], finished["cached_tokens"], finished["prefilled_tokens"]),
                                 (40, 30, 10))
                engine.close()
                # a fresh engine (restart) replays the stored prefix before serving
                _UpstreamHandler.seen = []
                with patch("openai_server.ARCH", "qwen38"):
                    second = ProxyEngine("vllm", "qwen3.8-flash-next-aider", env=env, spawn=False)
    ...
    def test_vllm_backend_takes_its_launcher_environment(self):
        env = {"COLI_VLLM_CMD": "/bin/bash /srv/start_qwen.sh", "COLI_VLLM_MODEL": "/srv/models/q27",
               "COLI_VLLM_ENV": "PORT=8082 HOST=127.0.0.1 CTX=fast PREFIX_CACHE=1",
               "COLI_VLLM_CONTEXT": "65536"}
        with patch("openai_server.ARCH", "qwen38"):
            engine = ProxyEngine("vllm", "qwen3.8-flash-next-aider", env=env, spawn=False)
        self.assertEqual(engine.command, ["/bin/bash", "/srv/start_qwen.sh"])
        self.assertEqual(engine.model_id, "qwen3.8-flash-next-aider")   # public id, spoofed
        with patch("openai_server.ARCH", "qwen38"):
            poisoned = ProxyEngine("vllm", "qwen3.8-flash-next-aider",
                                   env=dict(env, PYTHONPATH="/venvs/hf/lib/python3.14/site-packages"), spawn=False)
        self.assertNotIn("PYTHONPATH", poisoned.child_env)
        poisoned.close()
        self.assertEqual(engine.upstream_model, "qwen3.8-27b")           # launcher's served name
        self.assertEqual(engine.backend["model"], "q27")
        self.assertEqual(engine.context_window, 65536)
    ...
    def test_backend_setting_persists_for_the_next_restart(self):
        with tempfile.TemporaryDirectory() as tmp, \
             patch.dict("os.environ", {"COLI_LLAMACPP_MODEL": "/models/x.gguf"}):
            path = Path(tmp) / "settings.json"
            first = APIServer(("127.0.0.1", 0), FakeEngine(), "test-model", settings_file=path)
            try:
                payload = first.update_settings({"backend": "llamacpp"})
                self.assertEqual(payload["backend"], "aider")
                self.assertEqual(payload["backend_next"], "llamacpp")
                self.assertEqual(payload["backends"], ["aider", "llamacpp"])
                with self.assertRaises(APIError):
                    first.update_settings({"backend": "vllm"})
            finally:
                first.scheduler.close(); first.server_close()
            self.assertEqual(saved_backend(path), "llamacpp")
