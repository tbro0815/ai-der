    ...
    const char *line, ColiServeCommand *command)
{
    ...
    else if (name_size == 5 && !memcmp(name, "RESET", 5))
        command->kind = COLI_SERVE_COMMAND_RESET;
    else
        return;
    while (*cursor == ' ' || *cursor == '\t') cursor++;
    ...
    void *(*allocate)(size_t))
{
    ...
    else if (!strcmp(fields[0], "RESET")) command->kind = COLI_SERVE_COMMAND_RESET;
    else {
        free(line);
        return COLI_SERVE_READ_IGNORED;
    }
    ...
    if (command->kind == COLI_SERVE_COMMAND_RESET) {
        if (nfields != 3 || !coli_serve_parse_i32(fields[2], &command->slot)) {
            free(line);
            return COLI_SERVE_READ_BAD_REQUEST;
        }
        free(line);
...
/* Request-scoped prefill telemetry. `cached_tokens` is the exact prompt
 * prefix restored from the KV pool. */
static inline int coli_serve_write_prefill(
    FILE *output, const char *id, int prompt_tokens, int cached_tokens,
    double elapsed_seconds)
{
    return fprintf(output, "PREFILL %s %d %d %.6f\n", id, prompt_tokens,
                   cached_tokens, elapsed_seconds) >= 0 &&
           fflush(output) == 0;
}

static inline int coli_serve_write_reset(FILE *output, const char *id)
{
    return fprintf(output, "RESET %s OK\n", id) >= 0 && fflush(output) == 0;
}
